...
# Get the test split of stead
data = sbd.STEAD(sampling_rate=100)
_, _, test = data.train_dev_test()
mask = test.metadata["trace_category"] == 'earthquake_local' 
test.filter(mask)
...
test_generator = GenericGenerator(test, Window_length, 99, None)
...
test_loader = DataLoader(test_generator, batch_size=1, shuffle=False, num_workers=2, worker_init_fn=worker_seeding)
